...
contract FixedTermPricing is Pricing {
    ...
    function getPaymentConsideration(LoanManager.Loan calldata loan)
        external
        view
        virtual
        override
    ...
    function getOwed(LoanManager.Loan calldata loan) public view override returns (uint256[] memory) {
        ...
        returns (uint256[] memory updatedDebt)
    {
        ...
        for (uint256 i = 0; i < loan.debt.length; i++) {
            updatedDebt[i] = loan.debt[i].amount + _getInterest(loan, details, timestamp, i);
        }
    }

    function _getInterest(LoanManager.Loan memory loan, Details memory details, uint256 timestamp, uint256 index)
        internal
        pure
        returns (uint256)
    {
