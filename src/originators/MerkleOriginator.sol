...
contract MerkleOriginator is Originator {
    ...
    function _validateMerkleProof(MerkleProof memory incomingMerkleProof, bytes32 leafHash) internal view virtual {
        if (!MerkleProofLib.verify(incomingMerkleProof.proof, incomingMerkleProof.root, leafHash)) {
    ...
    function execute(Request calldata params) external override returns (Response memory response) {
        ...
        _validateSignature(keccak256(encodeWithAccountCounter(strategist, proof.root)), params.signature);
