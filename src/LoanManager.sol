...
contract LoanManager is ERC721, ContractOffererInterface, TokenReceiverInterface {
    ...
    function previewOrder(
        ...
        Action action = _getAction(context);

        if (action == Action.LOCK) {
            (offer, consideration) = _previewLock(caller, fulfiller, minimumReceived, maximumSpent, context);
        } else if (action == Action.UNLOCK) {
            (offer, consideration) = _previewUnlock(caller, fulfiller, minimumReceived, maximumSpent, context);
        } else {
    ...
    function _getAction(bytes calldata context) internal pure returns (Action action) {
        assembly {
    ...
    function _executeLock(ReceivedItem[] calldata consideration, bytes calldata context) internal {
        ...
        for (; i < nlrs.length;) {
            ...
            if (
                ConduitInterface.execute.selector
                    != ConduitInterface(response.conduit).execute(_packageTransfers(loan, response.lender))
            ) {
