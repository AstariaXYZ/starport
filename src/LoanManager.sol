...
contract LoanManager is ERC721, ContractOffererInterface {
    ...
    function previewOrder(
        ...
        if (caller != address(seaport)) {
            ...
            offer[0] = SpentItem({
                ...
                token: address(this),
                identifier: uint256(keccak256(abi.encode(loan))),
                amount: 1
            });
        }
    ...
    function generateOrder(
        ...
        LoanManager.NewLoanRequest memory nlr = abi.decode(context, (LoanManager.NewLoanRequest));
    ...
    function _setCollateral(ReceivedItem[] memory consideration)
        internal
        pure
        returns (SpentItem[] memory collateral)
    {
        //spent item
...
contract Custodian is ContractOffererInterface, TokenReceiverInterface {
    ...
    function ratifyOrder(
        ...
        if (SettlementHandler.execute.selector != SettlementHandler(loan.terms.handler).execute(loan)) {
    ...
    // PUBLIC FUNCTIONS
    function onERC721Received(address, address, uint256, bytes calldata) public pure virtual returns (bytes4) {
