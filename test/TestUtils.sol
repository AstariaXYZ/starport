...
contract TestStarLiteUtils is Test {
  ...
  function testSpentToReceived() public {
    ...
    ReceivedItem[] memory consideration0 = testContract.spentToReceivedBoring(
      spentItems,
      address(1)
    );

    ReceivedItem[] memory consideration1 = testContract.spentToReceivedSexc(
      spentItems,
      address(1)
    );
  ...
  function testEncodeReceivedWithRecipient() public {
    ...
    ReceivedItem[] memory consideration0 = testContract
      .encodeReceivedItemsWithRecipientBoring(receivedItems, address(1));

    ReceivedItem[] memory consideration1 = testContract
      .encodeReceivedItemsWithRecipientSexc(receivedItems, address(1));
...
contract TestContract {
  ...
  function encodeReceivedItemsWithRecipientSexc(
    ReceivedItem[] calldata receivedItems,
    address recipient
  ) external pure returns (ReceivedItem[] memory consideration) {
    return receivedItems.encodeWithRecipient(recipient);
  }

  function encodeReceivedItemsWithRecipientBoring(
    ReceivedItem[] calldata receivedItems,
    address recipient
  ) external pure returns (ReceivedItem[] memory consideration) {
  ...
  function spentToReceivedBoring(
    SpentItem[] calldata spentItems,
    address recipient
  ) external pure returns (ReceivedItem[] memory consideration) {
