...
contract TestNewLoan is StarPortTest {
  function testNewLoanERC721CollateralDefaultTerms2() public returns (LoanManager.Loan memory) {
    ...
    return newLoan(NewLoanData(address(custody), caveats, abi.encode(loanDetails)), Originator(UO), selectedCollateral);
  }
  ...
  function testSettleLoan() public {
    ...
    if (restricted == address(0)) {
      AdvancedOrder memory settlementOrder =
        AdvancedOrder({numerator: 1, denominator: 1, parameters: op, extraData: abi.encode(activeLoan), signature: ""});
