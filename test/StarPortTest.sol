...
import {
  ItemType, ReceivedItem, OfferItem, SpentItem, OrderParameters
...
contract StarPortTest is BaseOrderTest {
  ...
  bytes defaultHandlerData =
    abi.encode(DutchAuctionHandler.Details({startingPrice: uint256(500 ether), endingPrice: 100 wei, window: 7 days}));
  ...
  function newLoanWithMerkleProof(
    ...
    bool isTrusted = loanData.caveats.length == 0;
    {
      ...
      MerkleOriginator.MerkleProof memory merkleData = abi.decode(details.validator, (MerkleOriginator.MerkleProof));
      (uint8 v, bytes32 r, bytes32 s) =
        vm.sign(strategist.key, keccak256(originator.encodeWithAccountCounter(strategist.addr, merkleData.root)));
  ...
  function _executeRepayLoan(LoanManager.Loan memory activeLoan) internal {
    ...
    AdvancedOrder memory x =
      AdvancedOrder({parameters: op, numerator: 1, denominator: 1, signature: "0x", extraData: abi.encode(activeLoan)});
